...
def make_ind():
    ...
    ones=np.ones(100)
    change=np.random.choice(range(100),3,replace=False)
    for i in change:
        ones[i]=0
    ind=ones.tolist()
    return ind
...
def mutate(ind,prob):
    ...
    if np.random.rand()<=prob:
        ...
        k=np.random.choice(ones_loc)
        j=np.random.choice(zero_loc)
...
def select(pop,k):
    ...
    for _ in range(len(pop)):
        temp_index=np.random.choice(len(pop),k,replace=False)
        temp_pop=[pop[i] for i in temp_index]
